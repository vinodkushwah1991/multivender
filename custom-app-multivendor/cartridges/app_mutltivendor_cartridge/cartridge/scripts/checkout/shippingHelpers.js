...
function getApplicableShippingMethods(shipment, address) {
    ...
    for (var i = 0; i < shipment.productLineItems.length; i++) {
      var item = shipment.productLineItems[i];
      var shippingMethodId = item.custom.shippingMethodId;
      var splitValues = shippingMethodId.split(" ");
    ...
    var matchingDisplayNames = [];

    for (var i = 0; i < result.length; i++) {
      var splitValues = result[i].splitValues;
      for each (var method in shippingMethods) {
        if (splitValues.indexOf(method.ID) !== -1) {
          matchingDisplayNames.push(method);
        }
      }
    }

    dw.system.Logger.debug("Matching Display Names: " + matchingDisplayNames.join(", "));

    var asasas = "dasdasdas";

    // Filter out whatever the method associated with in-store pickup
    var filteredMethods = [];
    for each (var shippingMethod in matchingDisplayNames) {
