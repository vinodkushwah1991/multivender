...
var ProductInventoryList = require("dw/catalog/ProductInventoryList");
...
function addLineItem(
  ...
  try {
    productLineItem.custom.priceListId = cartFromdata.pricebookids;
    productLineItem.custom.sellerRefId = cartFromdata.refid;
    productLineItem.custom.inventoryListId = cartFromdata.inventoryListId;
    productLineItem.custom.shippingMethodId = cartFromdata.shippingmethodid;
    productLineItem.custom.companyName = cartFromdata.companyName;
    productLineItem.custom.fromStoreId = storeId ? storeId : "" ;
  } catch (error) {
...
function getExistingProductLineItemInCart(
  ...
  // Call a function to get existing product line items that match the initial criteria
  ...
  // Check if 'refid' is present in the cartFromdata object
  if ("refid" in cartFromdata) {
    // Filter the matchingProducts array to include only items with custom.sellerRefId matching cartFromdata.refid
    matchingProducts = matchingProducts.filter(function (matchingProduct) {
      return matchingProduct.custom.sellerRefId === cartFromdata.refid;
    });
  }

  // Return the first matching product line item (or undefined if none found)
...
function addProductToCart(
  ...
  var ab = currentBasket;
  var ac = productId;
  var ad = quantity;
  var ae = childProducts;
  var af = options;
  var ag = cartFromdata;
  var seller_added_to_cart = testval2;

  var sellerSelected = testval2;

  var inventoryId = `inventory_${sellerSelected}_${locale}`;
  var availableToSell;
  var UUIDUtils = require("dw/util/UUIDUtils");
  var uuid = UUIDUtils.createUUID();
  var defaultShipment = currentBasket.createShipment(uuid);
  var perpetual;
  var product = ProductMgr.getProduct(productId);
  var inventoryRecord = ProductInventoryMgr.getInventoryList(inventoryId)
    ? ProductInventoryMgr.getInventoryList(inventoryId).getRecord(productId)
    : "";
  var productInCart;
  ...
  if(sellerSelected){
  var storeId = `store_${sellerSelected}_${locale}`;
}
  var result = {
    error: false,
  ...
  /* Setting up the individual pricebook for the product */

  var CustomObjectMgr = require("dw/object/CustomObjectMgr");
  var PriceBookMgr = require("dw/catalog/PriceBookMgr");

  var siteLocal = locale;
  ...
  var localeMapping = localeAndCurrency.find((mapping) => siteLocal in mapping);
  ...
  var sellerData = CustomObjectMgr.getCustomObject(
    "productSellerRef",
    seller_added_to_cart
  );
  var pricebookIds = sellerData ? sellerData.custom.pricebookIds:"" ;
  ...
  var pricebook = filteredPricebookIds[0];

  PriceBookMgr.setApplicablePriceBooks(PriceBookMgr.getPriceBook(pricebook));

  Transaction.wrap(function () {
    collections.forEach(
      currentBasket.allProductLineItems,
      function (productLineItem) {
        var a = productLineItem;
        productLineItem.setPriceValue(productPriceFinalLocale);
        var b = productLineItem;
        var val = 10;
      }
    );
  });
  ...
  if (inventoryRecord) {
    if (product.bundle) {
      ...
      perpetual = inventoryRecord.ATS.value > 0 ? true : false;
      canBeAdded = perpetual || totalQtyRequested <= inventoryRecord.ATS.value;
    }
    if (!canBeAdded) {
        result.error = true;
