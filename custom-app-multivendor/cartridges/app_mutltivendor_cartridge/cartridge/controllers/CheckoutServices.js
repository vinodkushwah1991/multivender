...
server.post("PlaceOrder", server.middleware.https, function (req, res, next) {
  ...
  var Transaction = require("dw/system/Transaction");
  var URLUtils = require("dw/web/URLUtils");
  var basketCalculationHelpers = require("*/cartridge/scripts/helpers/basketCalculationHelpers"); // nothing
  var hooksHelper = require("*/cartridge/scripts/helpers/hooks");
  var COHelpers = require("*/cartridge/scripts/checkout/checkoutHelpers");
  var validationHelpers = require("*/cartridge/scripts/helpers/basketValidationHelpers");
  ...
  Transaction.wrap(function () {
    collections.forEach(
      ...
      function (productLineItem) {
        ...
        var storeId = `${productLineItem.custom.fromStoreId}`; //store_sellerId_locale
        if (storeId) {
          var store = StoreMgr.getStore(storeId);

          if (store) {
            var storeinventory = ProductInventoryMgr.getInventoryList(
              productLineItem.custom.inventoryListId
            );
            if (storeinventory) {
              if (
  ...
  // this is the base method. need to create new method and check the inventoryAvailability in our custom inventory.
  var validatedProducts = validationHelpers.validateProducts(currentBasket);
  if (validatedProducts.error) {
    res.json({
      error: true,
