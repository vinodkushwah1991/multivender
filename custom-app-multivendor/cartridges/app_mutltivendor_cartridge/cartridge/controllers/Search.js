...
server.get('Price', function (req, res, next) {
    ...
    // var ProductPriceInfo = require('dw/catalog/ProductPriceInfo');
    var productId = 'sports-cricket-001';
    // var a = ProductPriceInfo.getPrice(productId);
    ...
    if (product) {
      // Get the site price books
      var sitePriceBooks = PriceBookMgr.getSitePriceBooks();
  
      // Create an array to store the results
      var prices = [];
  
      // Iterate through the site price books
      for (var i = 0; i < sitePriceBooks.length; i++) {
        var priceBook = sitePriceBooks[i].ID;
  
        // Get the price model for the product in the current price book
        var priceModel = product.getPriceModel();
        var test = priceModel.getPriceBookPrice(priceBook);
  
        // Check if a valid price is available in the current price book
        if (priceModel.getPrice() !== null) {
          var productPrice = priceModel.getPrice().getValue();
          var currencyCode = priceModel.getPrice().getCurrencyCode();
  
          // Add the price information to the results array
          prices.push({
            test: test.value,
            priceBook: priceBook
          });
      ...
      // Send the array of price information as JSON response
      res.json(prices);
      res.json({
              test: test,
            });
...
server.get('Show', cache.applyShortPromotionSensitiveCache, consentTracking.consent, function (req, res, next) {
    ...
    var productids = result.productSearch.productIds; // Assuming productids is an array of product IDs
    ...
    for (var i = 0; i < productids.length; i++) {
      ...
      // Get the product for the current product ID
      var product = ProductMgr.getProduct(productId.productID);

      // Check if the product exists
      if (product) {
        for (var j = 0; j < sitePriceBooks.length; j++) {
          var priceBook = sitePriceBooks[j].ID;
