...
function getCheckoutStepInformation(lineItemContainer) {
    ...
    return {
        ...
        billing: {
            iscompleted: !!lineItemContainer.billingAddress
        }
    };
}
...
function OrderModel(lineItemContainer, options) {
    ...
    if (!lineItemContainer) {
        ...
        for (var modelIndex = 0; modelIndex < shippingModels.length; modelIndex++) {
            var customShippingMethods = shippingModels[modelIndex].productLineItems.items;
            var shipArray = [];
        
            for (var i = 0; i < customShippingMethods.length; i++) {
                var shippingValue = customShippingMethods[i];
                var dataValue = shippingValue.shippingMethodId;
        
                // Additional values to store in shipArray
                var id = shippingValue.id;
                var sellerRefId = shippingValue.sellerRefId;
                var shipmentUUID = shippingValue.shipmentUUID;
            ...
            for (var i = 0; i < shipArray.length; i++) {
                var stringShipMethod = shipArray[i].dataValue;
                var splitValues = stringShipMethod.split(' ');
        
                var obj = {
                    firstValue: splitValues[0],
                    secondValue: splitValues[1],
                    thirdValue: splitValues[2],
                    fourthValue: splitValues[3]
                };
        
                resultArray.push(obj);
            }
        
            var applicableShippingMethods = shippingModels[modelIndex].applicableShippingMethods;
            var modelUUID = shippingModels[modelIndex].UUID;
        
            // Create an object to store matching values for the same UUID
            var matchingObject = {
                UUID: modelUUID,
                matchingValues: []
            };
            ...
            for (var i = 0; i < applicableShippingMethods.length; i++) {
                var idToMatch = applicableShippingMethods[i].ID;
        
                for (var j = 0; j < resultArray.length; j++) {
                    var resultObj = resultArray[j];
        
                    if (
                        resultObj.firstValue == idToMatch ||
                        resultObj.secondValue == idToMatch ||
                        resultObj.thirdValue == idToMatch ||
                        resultObj.fourthValue == idToMatch
                    ) {
                        if (resultObj.firstValue == idToMatch) {
                            matchingObject.matchingValues.push(resultObj.firstValue);
                        }
                        if (resultObj.secondValue == idToMatch) {
                            matchingObject.matchingValues.push(resultObj.secondValue);
                        }
                        if (resultObj.thirdValue == idToMatch) {
                            matchingObject.matchingValues.push(resultObj.thirdValue);
                        }
                        if (resultObj.fourthValue == idToMatch) {
                            matchingObject.matchingValues.push(resultObj.fourthValue);
                        }
                    }
                }
            }
        
            // Add the matchingObject to matchingValues only if it has matching values
            if (matchingObject.matchingValues.length > 0) {
                matchingValues.push(matchingObject);
            }
        }
