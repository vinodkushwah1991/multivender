...
function calculateProductPrices (basket) {
    ...
    while (productQuantitiesIt.hasNext()) {
        ...
        var prodLineItem = basket.getProductLineItems(prod.ID);
        var priceListId= prodLineItem[0].custom.priceListId;
        if (priceListId) {
                    PriceBookMgr.setApplicablePriceBooks(PriceBookMgr.getPriceBook(priceListId))
                }

        // while (productLineItems.hasNext()) {
        //     var productLineItem = productLineItems.next();
        //     // var priceListId = productLineItem.custom.priceListId;
        //     PriceBookMgr.setApplicablePriceBooks();
        //     if (prod.ID) {
        //         PriceBookMgr.setApplicablePriceBooks(PriceBookMgr.getPriceBook(productLineItem.custom.priceListId))
        //     }
    ...
    while (productLineItems.hasNext()) {
        var productLineItem = productLineItems.next();
        // PriceBookMgr.setApplicablePriceBooks();
        //   if (productLineItem.productID) {
        //     PriceBookMgr.setApplicablePriceBooks(PriceBookMgr.getPriceBook(productLineItem.custom.priceListId))
        // }
        // PriceBookMgr.setApplicablePriceBooks()
        // // productLineItem.custom.priceListId
